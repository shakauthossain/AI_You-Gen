        ...
        <div className="text-center">
          ...
          <div className="text-xs text-muted-foreground space-y-2">
            <div className="flex justify-between items-center py-1 px-2 rounded bg-muted/50">
              ...
              <span className="font-medium text-foreground">{state.conversations.length}</span>
            </div>
            <div className="flex justify-between items-center py-1 px-2 rounded bg-muted/50">
              <span>Total Snippets:</span>
              <span className="font-medium text-foreground">{state.allSnippets.length}</span>
            </div>
            <div className="flex justify-between items-center py-1 px-2 rounded bg-muted/50">
              <span>MCQ Sets:</span>
              <span className="font-medium text-foreground">{state.mcqSets.length}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="text-center space-y-4">
          {state.transcriptStatus === 'loaded' && (
            <div className="bg-success/10 border border-success/20 rounded-lg p-3">
              <div className="text-xs text-success font-medium">✓ Video loaded successfully</div>
          ...
          {state.transcriptStatus === 'loading' && (
            <div className="bg-primary/10 border border-primary/20 rounded-lg p-3">
              <div className="text-xs text-primary font-medium">⏳ Processing video...</div>
              <div className="text-xs text-muted-foreground mt-1">
          ...
          {state.transcriptStatus === 'error' && (
            <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3">
              <div className="text-xs text-destructive font-medium">⚠ Processing failed</div>
              <div className="text-xs text-muted-foreground mt-1">
