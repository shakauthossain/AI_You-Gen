...
# LangChain LLM
chat = ChatGroq(
    api_key="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",
...
# ---------------- YouTube Section ----------------
st.markdown("## 1. YouTube Transcript Based Q&A and MCQs")
...
st.markdown("### Generate MCQs from YouTube Transcript")
with st.container():
    col5, col6 = st.columns([6, 1])
    with col5:
        num_mcqs_youtube = st.slider("Number of MCQs", 1, 10, 5, label_visibility="collapsed")
    with col6:
        generate_youtube_mcqs = st.button("Generate YouTube MCQs", use_container_width=True)

if generate_youtube_mcqs:
...
# ---------------- File Upload Section ----------------
st.markdown("---")
st.markdown("## 2. File Based MCQ Generator")

uploaded_file = st.file_uploader("Upload a TXT, PDF or DOCX file", type=["txt", "pdf", "docx"])
num_mcqs_file = st.slider("Number of MCQs to generate from file", 1, 10, 5)
